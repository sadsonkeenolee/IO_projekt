...
create procedure find_movie_id(in q_title varchar(255))
       begin
	  select tmdb_id 
	  from movies m
...
create procedure get_book_by_id (in book_id bigint unsigned)
       begin
	  select 
...
create procedure get_book_authors(in book_id bigint unsigned)
      begin
...
create procedure find_book_id (in title varchar(255))
       begin
	  select ID	  
	  from books b 
